...
async function handleCheckAnswers() {
    ...
    if (fractionAppAuth && fractionAppAuth.isAuthenticated()) {
        ...
        try {
            const saved = await fractionAppAuth.saveWorksheetResults(worksheetData);
            if (saved) {
                console.log('✅ Worksheet results saved successfully');
            } else {
                console.warn('⚠️ Worksheet results may not have been saved');
                if (fractionAppAuth.showNotification) {
                    fractionAppAuth.showNotification('Worksheet results could not be saved', 'warning', 3000);
                }
            }
        } catch (error) {
            console.error('❌ Failed to save worksheet results:', error);
            if (fractionAppAuth.showNotification) {
                fractionAppAuth.showNotification('Failed to save worksheet. Please check your connection.', 'error', 4000);
            }
        }
    } else {
        // User is not signed in - show informational message
        console.log('ℹ️ Not signed in, worksheet not saved to cloud');
        if (fractionAppAuth && fractionAppAuth.showNotification) {
            fractionAppAuth.showNotification('Sign in to save your progress to the cloud', 'info', 3000);
        }
    }
}
