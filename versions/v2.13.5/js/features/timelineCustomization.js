...
export function initializeTimelineCustomization() {
    ...
    // Add customization button - try multiple times to handle async loading
    const tryAddButton = (attempts = 0) => {
        const added = addCustomizationButton();
        if (!added && attempts < 5) {
...
function addCustomizationButton() {
    ...
    // Find the educational content section
    const educationalContent = document.querySelector('.educational-content .collapsible-content');
    if (!educationalContent) {
        console.warn('Educational content section not found for timeline button');
        return false;
    }
    ...
    // Find the first h3 heading for "Brief History of Fractions"
    const historyHeading = Array.from(educationalContent.querySelectorAll('h3'))
        .find(h3 => h3.textContent.includes('Brief History'));

    if (historyHeading) {
        // Insert button right before the "Brief History" heading
        historyHeading.parentNode.insertBefore(button, historyHeading);
        console.log('✅ Timeline customization button added');
        return true;
    } else {
        // Fallback: insert before the first carousel-container
        const firstCarousel = educationalContent.querySelector('.carousel-container');
        if (firstCarousel) {
