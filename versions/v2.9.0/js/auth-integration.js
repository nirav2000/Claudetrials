...
class FractionAppAuth {
  ...
  showLoginModal() {
    ...
    try {
      ...
      // Show popup blocker warning for iPad users
      this.showNotification(
        'If the sign-in window is blocked, please allow popups for this site and try again.',
        'info',
        4000
      );
      ...
      authUI.show({
        ...
        onSuccess: (result) => {
          ...
          // Show welcome message with popup close instruction
          this.showNotification(
            `Welcome ${user.displayName || 'back'}! Your progress will now be saved to the cloud. (You can close the sign-in window if it's still open)`,
            'success',
            6000
          );
        ...
        onError: (error) => {
          ...
          if (error.message && error.message.includes('popup')) {
            errorMsg = 'Sign-in window issue. Please allow popups and try again.';
  ...
  async loadUserProgress(userId) {
    ...
    try {
      ...
      if (result.success && result.data.progress) {
        ...
        // Restore last worksheet settings
        if (progress.lastWorksheet && progress.lastWorksheet.settings) {
          this.restoreSettings(progress.lastWorksheet.settings);
        }
  ...
  /**
   * Restore worksheet settings to the UI
    ...
    try {
      ...
      if (settings.problemCount) {
        ...
        if (problemCountSelect) {
          problemCountSelect.value = settings.problemCount;
        }
      }

      // Restore denominator set (difficulty)
      if (settings.denominatorSet) {
        const denominatorSetSelect = document.getElementById('denominator-set');
        if (denominatorSetSelect) {
          denominatorSetSelect.value = settings.denominatorSet;
        }
      }
      ...
      // Restore hint timeout
      if (settings.hintTimeout !== undefined) {
        const hintTimeoutSelect = document.getElementById('hint-timeout');
        if (hintTimeoutSelect) {
          hintTimeoutSelect.value = settings.hintTimeout;
        }
      }

      // Restore common mistakes toggle
      if (settings.useCommonMistakes !== undefined) {
        const commonMistakesCheckbox = document.getElementById('common-mistakes');
        if (commonMistakesCheckbox) {
          commonMistakesCheckbox.checked = settings.useCommonMistakes;

          // Show/hide mistake types section
          const mistakeTypesSection = document.getElementById('mistake-types-section');
          if (mistakeTypesSection) {
            mistakeTypesSection.classList.toggle('show', settings.useCommonMistakes);
          }
        }
      }

      // Restore enabled mistake types
      if (settings.enabledMistakeTypes && settings.enabledMistakeTypes.length > 0) {
        // First, uncheck all mistake type checkboxes
        const mistakeCheckboxes = document.querySelectorAll('.mistake-type-checkbox');
        mistakeCheckboxes.forEach(cb => {
          cb.checked = false;
        });

        // Then check the enabled ones
        settings.enabledMistakeTypes.forEach(mistakeType => {
          const checkbox = document.querySelector(`.mistake-type-checkbox[value="${mistakeType}"]`);
          if (checkbox) {
      ...
      this.showNotification('Your previous settings have been restored', 'success', 3000);
   ...
   * @param {Object} results - Worksheet results with worksheetId, problems, score, settings, timeSpent, completedAt
   */
  async saveWorksheetResults(results) {
    if (!this.currentUser) {
      console.log('Not signed in, worksheet not saved');
      return false;
    }

    if (!authManager || !authManager.db) {
      console.error('Cannot save worksheet: authManager not loaded');
      return false;
    }
    ...
    try {
      ...
      const worksheetId = results.worksheetId || `worksheet_${Date.now()}`;

      // Create full worksheet document
      const worksheetData = {
        userId: userId,
        ...
        score: results.score || { correct: 0, incorrect: 0, unanswered: 0, total: 0, percentage: 0 },
        timeSpent: results.timeSpent || 0
      };

      // Save worksheet to worksheets collection
      await db.collection('worksheets').doc(worksheetId).set(worksheetData);

      console.log('✅ Worksheet saved to collection:', worksheetId);

      // Update user's progress document
      const totalWorksheets = (await this.getTotalWorksheets()) + 1;
      ...
      this.showNotification('Failed to save worksheet', 'error', 3000);
      return false;
    }
  }
   ...
   * @param {number} limit - Maximum number of worksheets to retrieve
   * @returns {Promise<Array>} Array of worksheet objects
   */
  async getWorksheetHistory(limit = 10) {
    if (!this.currentUser) return [];
    if (!authManager || !authManager.db) return [];

    try {
      const db = authManager.db;
      const userId = this.currentUser.uid;

      // Query worksheets collection for this user, ordered by completion date
      const snapshot = await db.collection('worksheets')
        .where('userId', '==', userId)
      ...
      console.error('Failed to get worksheet history:', error);
      return [];
    }
  }
  ...
  async showUserProgress() {
    ...
    try {
      const result = await authManager.getUserData(this.currentUser.uid);
      const progress = result.data?.progress || {};
      const worksheets = await this.getWorksheetHistory(10);
          ...
          <div class="auth-modal-content">
            ...
            <div style="margin-top: 24px;">
              ...
              ${progress.lastWorksheet ? `
                <div class="auth-progress-stat">
                  ...
                  <div class="auth-progress-value">${progress.lastWorksheet.score?.correct || 0} / ${progress.lastWorksheet.score?.total || 0} (${progress.lastWorksheet.score?.percentage || 0}%)</div>
                </div>
                <div class="auth-progress-stat">
            ...
            ${worksheets.length > 0 ? `
              <div style="margin-top: 32px;">
                <h3 style="margin-bottom: 16px; color: #333;">Recent Worksheets</h3>
                <div class="auth-worksheet-history">
                  ${worksheets.map((ws, index) => `
                    <div class="auth-worksheet-item" style="
                    ...
                    ">
                      <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                        <div style="font-weight: 600; color: #333;">
                          Worksheet ${worksheets.length - index}
                          ${index === 0 ? '<span style="font-size: 11px; background: #4CAF50; color: white; padding: 2px 8px; border-radius: 4px; margin-left: 8px;">Latest</span>' : ''}
                        </div>
                        <div style="text-align: right;">
                          <div style="font-size: 20px; font-weight: 600; color: ${ws.score.percentage >= 80 ? '#4CAF50' : ws.score.percentage >= 60 ? '#FF9800' : '#f44336'};">
                            ${ws.score.percentage}%
                          </div>
                          <div style="font-size: 12px; color: #666;">
                            ${ws.score.correct}/${ws.score.total} correct
                          </div>
                        </div>
                      </div>
                      <div style="font-size: 13px; color: #666; margin-bottom: 8px;">
                        ${new Date(ws.completedAt).toLocaleString()}
                      </div>
                      <div style="font-size: 12px; color: #888; display: flex; flex-wrap: wrap; gap: 12px;">
                        <span>${ws.settings.problemCount || 0} problems</span>
                        <span>•</span>
                        <span>${ws.settings.denominatorSet || 'unknown'} difficulty</span>
                        ${ws.timeSpent ? `<span>•</span><span>${Math.floor(ws.timeSpent / 60)}m ${ws.timeSpent % 60}s</span>` : ''}
                      </div>
                    </div>
                  `).join('')}
                </div>
